...
def detect_seccion(filename: str) -> str | None:
    """Extrae sección del nombre buscando 2 dígitos que pertenezcan a Iveralso."""
    nombre = filename.upper()
    # Buscamos códigos de 2 dígitos NO precedidos por "0" inmediatamente (para evitar "00 -")
...
def parse(path: str | Path) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        ...
        if not isinstance(f, (dt.datetime, pd.Timestamp)):
            continue
        try:
            ingreso = float(row[col_ing[0]]) if pd.notna(row[col_ing[0]]) else 0.0
        except (TypeError, ValueError):
            ingreso = 0.0
        try:
            pago = float(row[col_pag[0]]) if col_pag and pd.notna(row[col_pag[0]]) else 0.0
        except (TypeError, ValueError):
            pago = 0.0
        rows.append({"fecha": f.date(), "ingreso": ingreso, "pago": pago})
